...
/*
 * Boot HUD for gemini: Wi-Fi + CPU/GPU/RAM.
 * Prefers GBM/EGL/GLES2 (Adreno 530); falls back to CPU write(/dev/fb0).
 * Volume Up (no power) still reboot-fastboot. GDM stays masked.
 */
#include "gemini-status-font.h"
#include "gemini-status-gpu.h"
#include "gemini-wifi-flow.h"
...
#define HUD_CONF "/var/lib/gemini/hud.conf"
...
static void logmsg(const char *msg)
{
	...
	snprintf(line, sizeof(line), "gemini-status: %s\n", msg);
	if (k >= 0) {
...
static void perf_dump(uint64_t now)
{
	...
	f = fopen("/run/gemini-hud-perf.txt", "w");
	if (f) {
		fprintf(f,
...
static void take_gpu_freq(long freq)
{
	...
	if (freq >= 1000000L)
		...
		return;
	/* Adreno 530 OPP is 133–624 MHz. Drop XO / misc clocks like 27 MHz. */
	if (mhz < 100 || mhz > 700)
		return;
	if (mhz > gpu_mhz)
		gpu_mhz = mhz;
...
static void wifi_connect(const char *ssid, const char *pass, const char *sec)
{
	...
	argv[0] = "/bin/sh";
	argv[1] = "/usr/local/sbin/gemini-wifi-connect.sh";
	argv[2] = s1;
	argv[3] = s2;
	argv[4] = s3;
	argv[5] = s4;
...
static int meters_h(void) { return 680; }
static int meters_y(void) { return H - 72 - meters_h(); }
...
{
	struct mt_slots_req req;

	memset(&req, 0, sizeof(req));
	req.code = code;
	if (ioctl(fd, EVIOCGMTSLOTS(sizeof(req)), &req) < 0)
		return -1;
	memcpy(out, req.values, sizeof(req.values));
	return 0;
}
...
static void mt_bind_primary(int idx)
{
	...
	if (contact_armed && mt_bind_slot >= 0 && mt_bind_slot < mt_nslots) {
		s = mt_bind_slot;
		if (mt_id[s] < 0 || !mt_have_y[s] || !mt_y[s])
			s = -1;
	} else {
		s = mt_finger(idx);
	}
...
static void draw_meters(void)
{
	...
	if (gpu_ok && gpu_mhz && gpu_temp_c > 0)
		snprintf(gpu_sub, sizeof(gpu_sub), "Adreno 530  ·  %d MHz  ·  %d℃",
			 gpu_mhz, gpu_temp_c);
	else if (gpu_ok && gpu_mhz)
		snprintf(gpu_sub, sizeof(gpu_sub), "Adreno 530  ·  %d MHz", gpu_mhz);
	else if (gpu_ok && gpu_temp_c > 0)
		snprintf(gpu_sub, sizeof(gpu_sub), "Adreno 530  ·  %d℃", gpu_temp_c);
	else if (gpu_ok)
		snprintf(gpu_sub, sizeof(gpu_sub), "Adreno 530");
	else
	...
	for (i = 0; i < cores && i < MAX_CPU; i++) {
		int yy = y + 212 + i * 68;
		snprintf(line, sizeof(line), "CPU%d", i);
		draw_text(&font_body, 56, yy + 4, line, px(180, 190, 205));
		draw_bar(bar_x, yy + 10, bar_w, 28, cpu_core_s[i] / 100.0f, px(91, 140, 255));
		snprintf(line, sizeof(line), "%.0f%%", cpu_core_t[i]);
		draw_text(&font_body, pct_x, yy + 8, line, px(200, 208, 220));
	}

	{
		int ry = y + 212 + cores * 68 + 24;
...
static void draw_home(void)
{
	...
	draw_text(&font_title, 48, 36, "XiaoMi 5", px(245, 248, 252));
	draw_lang_btn();
	fmt_uptime(up, sizeof(up), uptime_sec);
	draw_text(&font_body, 48, 96, "Ubuntu 26.04  ·  gemini", px(140, 150, 165));
	if (batt_ok && batt_pct >= 0)
