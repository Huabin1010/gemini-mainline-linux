			...
			bool is_bonded_dsi, struct msm_dsi_phy *phy)
{
	...
	/*
	 * msm8996 vdda/vcca/vddio feed the 14nm PHY analog. Dropping them
	 * on CRTC off forces DCS 0x11 on resume and paints JDI white bars.
	 * Enable once; host_power_off only gates link clocks.
	 */
	if (!msm_host->analog_held) {
		...
		msm_host->analog_held = true;
	}

	pm_runtime_get_sync(&msm_host->pdev->dev);
	/*
	 * pclk0_clk_src is a pixel RCG. CMD_UPDATE only latches once the
	 * DSI PHY PLL (parent) is running. A set_rate before enable WARNs
	 * "rcg didn't update" and leaves MDP/DSI on the LK leftover clock
	 * → backlight with a black command-mode panel.
	 */
	ret = cfg_hnd->ops->link_clk_set_rate(msm_host);
	if (ret)
		pr_warn("%s: link_clk_set_rate before PLL enable: %d\n",
			__func__, ret);
	...
	ret = cfg_hnd->ops->link_clk_set_rate(msm_host);
	if (ret) {
		pr_err("%s: link_clk_set_rate after PLL enable failed %d\n",
		       __func__, ret);
...
int msm_dsi_host_power_off(struct mipi_dsi_host *host)
{
	...
	/* Keep analog regulators and DSI pins; only the PLL is gated. */
