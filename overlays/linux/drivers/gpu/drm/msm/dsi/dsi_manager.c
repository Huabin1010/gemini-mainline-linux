...
static struct msm_dsi_phy_shared_timings saved_phy_timings[DSI_MAX];
		   ...
		   struct msm_dsi_phy_shared_timings shared_timings[DSI_MAX])
{
	...
	/*
	 * Analog-keep: do not reset_phy / re-enable if lanes are still up.
	 * Retrain + DCS 0x11 is the JDI R63452 right-edge white cluster.
	 */
	if (msm_dsi->phy_enabled) {
		shared_timings[id] = saved_phy_timings[id];
		return 0;
	}
	...
	saved_phy_timings[id] = shared_timings[id];
...
{
	/*
	 * Keep 14nm PHY analog and vcca across CRTC off so the JDI panel
	 * does not need DCS 0x11 on resume. Link clocks still gate in
