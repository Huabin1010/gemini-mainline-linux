...
static int wled4_setup(struct wled *wled)
{
	...
	if (sink_cfg == sink_en) {
		/*
		 * LK already programmed the sink mask. Still apply CABC:
		 * if CABC stays on and the panel is not outputting the
		 * CABC PWM, the strings stay dark with brightness>0.
		 */
				  ...
				  struct platform_device *pdev)
{
	...
	/*
	 * NO_AUTOEN keeps the line masked. LK often leaves WLED MOD_EN set
	 * for the splash; skipping disable_irq then made wled_ovp_work's
	 * enable_irq unbalanced (oops on gemini while taking over the panel).
	 */
	return 0;
}
