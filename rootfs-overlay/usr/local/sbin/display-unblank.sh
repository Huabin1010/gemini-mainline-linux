#!/bin/sh
# Load MSM DRM, unblock Wi-Fi, then run the status HUD.
set -eu

modprobe panel-jdi-fhd-r63452 >/dev/null 2>&1 || true
modprobe msm >/dev/null 2>&1 || true
modprobe ath10k_pci >/dev/null 2>&1 || true
...
for d in /sys/class/backlight/*; do
	...
	pct=""
	if [ -f /var/lib/gemini/hud.conf ]; then
		pct=$(awk -F= '/^bl=/{print $2; exit}' /var/lib/gemini/hud.conf 2>/dev/null || true)
	fi
...

{
	...
	ls /lib/firmware/ath10k/QCA6174/hw3.0 2>/dev/null || true
	cat /proc/bus/input/devices 2>/dev/null || true
} >> /var/log/gemini-display.log 2>&1 || true
